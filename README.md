...
A native Rust rewrite of the original C# app, focused on **NetEase Cloud Music**
only. See [`RUST_PORT_PLAN.md`](RUST_PORT_PLAN.md) for the design and the mapping
from the original C#.
...
the executable (`target/release/music-rpc.log` when running from cargo).
...
Rewrite of [Kxnrl/NetEase-Cloud-Music-DiscordRPC](https://github.com/Kxnrl/NetEase-Cloud-Music-DiscordRPC).
The memory patterns/offsets and the window-title approach derive from that project.
